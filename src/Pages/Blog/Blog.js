    ...
    return (
        ...
        <div>
            ...
            <div>
                ...
                <p>CORS stands for Cross-Origin Resource Sharing (CORS)
                simply it's all about the security applied to an API.
                it is an HTTP-header-based mechanism or Access-Control-Allow-Origin headers which specify which origins can access the API or that allows a server to indicate any origins (domain, scheme, or port) other than its own from which a browser should permit loading resources.</p>
            </div>
            <div>
                <h3>Why are people using `firebase`? What other options do have to implement authentication?</h3>
                <p>Firebase is basically a Google-backed app development platform.
                    core features – Build, Release & Monitor, Engage, authentication.Firebase is a popular cloud service and backend development platform, which is popular because of its advantages Some advantages:
<ol>
<li> Reliable & Extensive Databases (Realtime Database, Cloud Firestore)</li>
...
eBay Motors has adopted this Firebase ML Kit to improve user experience to localized searches and classify images.
            ...
            <div>
                ...
                <p>The private route component is similar to the public route, the only change is the redirect URL and authenticates condition. If the user is not authenticated he will be redirected to the login page and the user can only access the authenticated routes If he is authenticated (Logged in) The private route component is used to protect selected pages in a React app from unauthenticated users.</p>
            </div>
            <div>
                <h3>What is Node? How does Node work?</h3>
                <p>Node.js is a lean, fast, open-source backend javascript runtime environment. It is a used as backend service where javascript works on the server side of the application. This way javascript is used on both the front end and back end. Node.js runs on the chrome v8 engine which converts javascript code into machine code, it is highly scalable, lightweight, fast, and data-intensive.
Working of Node.js: Node.js accepts the request from the clients and sends the response, while working with the request node.js handles them with a single thread. To operate I/O operations or requests node.js use the concept of threads. A thread is a sequence of instructions that the server needs to perform. It runs parallel on the server to provide the information to multiple clients. Node.js is an event loop single-threaded language. It can handle concurrent requests with a single thread without blocking it for one request.
Node.js basically works on two concept
Asynchronous
Non-blocking I/O
Non-blocking I/o: Non-blocking i/o  means working with multiple requests without blocking the thread for a single request. I/O basically interacts with external systems such as files, and databases. Node.js is not used for CPU-intensive work means for calculations, and video processing because a single thread cannot handle the CPU works.
